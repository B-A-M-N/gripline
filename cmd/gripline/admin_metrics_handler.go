...
func adminMetrics(svc *control.Service, dp *proxy.DataPlane, governor resource.Authority, state *statebolt.Store, postgres *statepg.Store, spray *anomaly.Detector, observer *jsonlObserver, policyManager *policy.Manager, signer *terminator.Keyring, term *terminator.Terminator, adaptiveHealth []terminator.AdaptivePersistenceHealth) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if postgres != nil {
			...
			writeMetric("postgres_source_alias_canonical_identities", a.SourceAliasCanonicalIdentities)
			writeMetric("postgres_source_alias_rows", a.SourceAliasRows)
			writeMetric("postgres_source_alias_saturations_total", a.SourceAliasSaturations)
			writeMetric("postgres_source_alias_safe_evictions_total", a.SourceAliasSafeEvictions)
			writeMetric("postgres_source_alias_capacity_denials_total", a.SourceAliasCapacityDenials)
			writeMetric("postgres_credential_last_seen_batches_total", a.CredentialLastSeenBatches)
			writeMetric("postgres_credential_last_seen_rows_total", a.CredentialLastSeenRows)
			writeMetric("postgres_credential_last_seen_dropped_total", a.CredentialLastSeenDropped)
