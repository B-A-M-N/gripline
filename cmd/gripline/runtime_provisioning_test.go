...
func TestCLICredentialAddReadsClusterPolicyAndPepper(t *testing.T) {
	...
	cfgJSON := fmt.Sprintf(`{"listen":"127.0.0.1:8080","backend":{"url":"http://backend.invalid:80","trust_mode":"private_network","timeout":"5s"},"server":{"read_timeout":"5s","write_timeout":"5s","idle_timeout":"5s","read_header_timeout":"5s"},"identity":{"audience":"test-audience"},"tls":{"terminate_tls_upstream":true},"authority":{"backend":"postgres","dsn_env":"GRIPLINE_DSN","node_id":"cli-node","lease_ttl":"30s","renew_every":"5s"},"secrets":{"pepper_versions":{"1":%q,"2":%q}},"admin":{"listen":%q,"operator_tokens":{%q:"ops:credential.lifecycle,cluster.read,policy.read,policy.install"}},"paths":{"signer_keyring":"%s"}}`, testPepperEnv, secondPepper, strings.TrimPrefix(admin.URL, "http://"), token, filepath.Join(dir, "keyring.json"))
