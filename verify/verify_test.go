...
func testToken(t *testing.T, priv ed25519.PrivateKey, c Claims) string {
	...
	return base64.RawURLEncoding.EncodeToString(payload) + "." + base64.RawURLEncoding.EncodeToString(ed25519.Sign(priv, payload))
}
...
func TestConformanceVectors(t *testing.T) {
	...
	cases := []struct {
		...
		{"tampered", valid[:len(valid)-1] + "A", ErrBadSignature},
	}
...
func TestVerifyAndStripAndDuplicateClaims(t *testing.T) {
	...
	token := base64.RawURLEncoding.EncodeToString([]byte(payload)) + "." + base64.RawURLEncoding.EncodeToString(ed25519.Sign(priv, []byte(payload)))
